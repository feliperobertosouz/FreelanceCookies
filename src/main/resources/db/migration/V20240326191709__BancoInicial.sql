...
CREATE TABLE deposito_saque (
  id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
   ...
   conta INT NOT NULL,
   tempo_atual VARCHAR(255) NOT NULL,
...
CREATE TABLE oferta (
  id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
   ...
   tempo_atual DOUBLE PRECISION NOT NULL,
...
CREATE TABLE reclamacao (
  id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
   usuario_id INT,
   transacao_id INT,
   deposito_saque_id INT,
   texto VARCHAR(255) NOT NULL,
   tempo_atual VARCHAR(255),
   CONSTRAINT pk_reclamacao PRIMARY KEY (id)
);

ALTER TABLE reclamacao ADD CONSTRAINT FK_RECLAMACAO_ON_DEPOSITOSAQUE FOREIGN KEY (deposito_saque_id) REFERENCES deposito_saque (id);

ALTER TABLE reclamacao ADD CONSTRAINT FK_RECLAMACAO_ON_TRANSACAO FOREIGN KEY (transacao_id) REFERENCES transacao (id);
